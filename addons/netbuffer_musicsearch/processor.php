			...
			if(strlen($keyword)>=1){
				...
				if(null!=$result&&is_object($result)&&count($result->pagebean->contentlist)>0){
					...
					for($i=0;$i<$count;$i++){
						...
						array_push($news,array("picurl"=>$temp->albumpic_big,"title"=>(empty($temp->songname)||""==$temp->songname?"":"歌名:".$temp->songname).(empty($temp->singername)||""==$temp->singername?"":" 歌手:".$temp->singername)."\r\n专辑:".$temp->albumname,"url"=>$this->createMobileUrl("index",array("suri"=>$temp->m4a))));
					}
