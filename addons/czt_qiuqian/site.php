...
class Czt_qiuqianModuleSite extends WeModuleSite {
	...
	public function doMobileIndex() {
		...
		// require MODULE_ROOT."/jssdk.class.php";
		// $jssdkconfig = jssdk::get_sign();
		load()->classs('weixin.account');
		$accObj = WeiXinAccount::create($_W['acid']);//$_W['acid']
		$jssdkconfig = $accObj->getJssdkConfig();
		//unset($accObj);
		// dump($jssdkconfig);die();

		$num=count($qian)+count($settings['new_qian']);
	...
	public function doMobileQian() {
		...
		global $_W, $_GPC;
		$num=count($qian)+count($settings['new_qian']);
		if (count($settings['new_qian'])) {
			foreach ($settings['new_qian'] as $key => $value) {
				$qian[]=$settings['new_qian'][$key];
			}
		}
		$id=intval($_GPC['id']);
		if (!$id||!$qian[$id]) $id=mt_rand(0,$num-1);
		$qian=$qian[$id];
