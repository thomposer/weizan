...
class Xfeng_communityModuleSite extends WeModuleSite {
	...
	function __construct(){
		...
		if (empty($menus)) {
			...
			if ($data5) {
					...
					$nid5 = pdo_insertid();
					$m5 = array(
							array('pcate' => $nid5,'title' => '用户管理','url' => './index.php?c=site&a=entry&op=list&do=users&m=xfeng_community','uniacid' => $_W['uniacid'],'do' => 'perm','method' => 'perm'),
					);
	...
	public function NavMenu($do){
		...
		foreach ($menus as $key => $menu) {
			$m = pdo_fetchall("SELECT title,url,id,do,method FROM".tablename('xcommunity_menu')."WHERE pcate = :pcate AND uniacid =:uniacid $condition",array(':pcate' => $menu['id'],':uniacid' => $_W['uniacid']));
			$navmenus[] = array(
					'title' => $menu['title'],
					'items' => $m,
					'id' => $menu['id'],
