...
class finance {
	...
	public function pay($openid = '', $paytype = 0, $money = 0, $trade_no = '', $desc = '') {
		...
		$member = pdo_fetch("select uid from ".tablename('yun_fkz_member')." where openid='$openid'");
		if (empty($member)) {
		...
		if (empty($paytype)) {
			...
			//print_r($_W['cache']['unimodules:1:']['xkd_fkz']['config']);exit();
			$pay = $_W['cache']['unimodules:'.$_W['uniacid'].':'][$_GPC['m']]['config'];
			...
			$pars['amount'] = $money;
			$pars['desc'] = empty($desc)?'订单结算提现':$desc;
