...
if ($op == 'store') {
    ...
    $list = pdo_fetchall("SELECT a.id,b.store,a.paycode,a.username,a.fromuser,a.name,a.paytime,a.dateline,a.redtime,a.qrmoney,a.red,a.paytype,a.status,a.redmemo,a.cardmemo,a.getstatus FROM " . tablename('bm_payx_payed') . " a inner join " . tablename('bm_payx_reply') . " b on a.rid = b.rid WHERE b.weid='$weid' $condition ORDER BY id DESC");
} else {
    $rid = intval($_GPC['rid']);
    if (empty($rid)) {
        message('抱歉，传递的参数错误！', '', 'error');
    }
    $list = pdo_fetchall("SELECT a.id,b.store,a.paycode,a.username,a.fromuser,a.name,a.paytime,a.dateline,a.redtime,a.qrmoney,a.red,a.paytype,a.status,a.redmemo,a.cardmemo,a.getstatus FROM " . tablename('bm_payx_payed') . " a inner join " . tablename('bm_payx_reply') . " b on a.rid = b.rid WHERE b.rid = '$rid' $condition ORDER BY id DESC");
}
foreach ($list as &$row) {
...
echo "<td  style='color:red' align='center'>  <font size=4>所属店铺 </font></td>";
...
foreach ($list as $value) {
    ...
    echo "<td  align='center'>  <font size=4>" . $value['store'] . " </font></td>";
