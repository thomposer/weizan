...
function tpl_thumbnail($data){
	...
	foreach ($data as $da){
	    ...
	    <div class="thumbnail">
	      <img src="'.$da['icon'].'" alt="'.$da['title'].'">
	      <div class="caption">
	        <h3>'.$da['title'].'</h3>
	        <p>'.$da['description'].'</p>
	        <p><a href="'.$da['href'].'" class="btn btn-primary" role="button">'.$da['title'].'</a></p>
	      </div>
	    </div>
	  </div>';
	}
...
/**
*	@imeepos doc function tpl_nav($data = array(array('href'=>'','title'=>'')))
*	@return html $html
*/
function tpl_nav($data = array(array('href'=>'./?refersh','title'=>'demo','active'=>'false'))){
	if(empty($data)){
		return '';
	}
...
function tpl_select($data = array()){
